...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Kyouka AI Already Activated In This Chat")
            return
        await lel.edit(f"Kyouka AI Actived by {message.from_user.mention()} for users in {message.chat.title}")

    elif status == "OFF" or status == "off" or status == "Off":
        lel = await edit_or_reply(message, "`Processing...`")
        Escobar = remove_chat(int(message.chat.id))
        if not Escobar:
            await lel.edit("Kyouka AI Was Not Activated In This Chat")
            return
        await lel.edit(f"Kyouka AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}")
